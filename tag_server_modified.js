...
app.get('/query/:rand', function (req, res, next) {
    // Check access first
    var access_arr = decrypt(master_key, req.body.access);
    var now = new Date();
    var time = new Date([access_arr[1], access_arr[2], access_arr[3]].join(':'));
    var dif = Math.abs(now - time);

    // 16 hours
    if (access_arr[0] == master_key && dif < (1000 * 60 * 60 * 16)) {
        var c_0, c_1, c_2;
        N_r = req.params.rand;
